package org.fengsoft.jts2geojson.jts2geojsonweb;
...
@SpringBootTest
public class Jts2geojsonWebApplicationTests {
    ...
    public void testPoints() throws IOException {
        ...
        JtsMvt actual = MvtReader.loadMvt(new ByteArrayInputStream(bytes), new GeometryFactory(), new TagKeyValueMapConverter());
...
//
//        byte[] bytes = new MvtWriter.Builder().setLayer(layer1Name).add(point1).add(point2).setLayer(layer2Name).add(point3).build();
//
//        assertNotNull(bytes);
//
//        JtsMvt layers = MvtReader.loadMvt(new ByteArrayInputStream(bytes), new GeometryFactory(), new TagKeyValueMapConverter());
//
